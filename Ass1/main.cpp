...
BenchmarkResult run_simulation(int n, int rank, int size) {
    ...
    if (rank == 0) {
        printf("==============================================\n");
        printf("Start Hybrid simulation with matrix size %d x %d\n", n, n);
        printf("==============================================\n\n");
    }
    ...
    if (rank == 0) {
        R.mpi_naive = e1 - s1;
        print_block("Hybrid Parallel Naive",
                    R.mpi_naive,
                    verify_matrix_multiplication(C_seq_naive, C_seq_strassen, n));
    }
    ...
    if (rank == 0) {
        ...
        print_block("Hybrid Parallel Strassen",
                    R.mpi_strassen,
                    verify_matrix_multiplication(C_seq_strassen, C_mpi_strassen, n));
    }

    if (rank == 0) {
        printf("Finish Hybrid simulation %d x %d\n", n, n);
        printf("==============================================\n\n");
    }
...
void print_table(const vector<BenchmarkResult>& R) {
    printf("+--------+--------------+--------------+--------------+-----------------+\n");
    printf("| Size   | Seq Naive    | Seq Strassen | Hybrid Naive | Hybrid Strassen |\n");
    printf("+--------+--------------+--------------+--------------+-----------------+\n");
